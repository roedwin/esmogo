(function ($) {
	var total 			    = parseInt( topstorepagi.shop_infinite_total ) || '',
		count               = parseInt( topstorepagi.shop_infinite_count ) || '',
		ajax_url            = topstorepagi.ajax_url || '',
		shop_infinite_nonce = topstorepagi.shop_infinite_nonce || '',
		pagination          = topstorepagi.top_store_pagination || '',
		masonryEnabled      = false,
		loadStatus          = true,
		infinite_event      = topstorepagi.shop_infinite_scroll_event || '',
	...
	if( typeof pagination != '' || pagination == 'scroll' ) {
		...
		function NextloadArticles(pageNumber){
			...
			loader.show();
			var data = {
				action :'top_store_pagination_infinite',
				page_no	: pageNumber,
				nonce: shop_infinite_nonce,
				query_vars: topstorepagi.query_vars,
				opnaira_infinite:'top_store_pagination_ajax',
			}
			...
			$.post( ajax_url, data, function( data ){
				...
				//	Add grid classes
				var msg = topstorepagi.shop_no_more_post_message || '';
