...
if ( ! function_exists( 'top_store_post_link' ) ){
	...
	function top_store_post_link( $output_filter = '' ){
		...
		$top_store_pro_read_more_text = apply_filters( 'top_store_post_read_more', __( 'Read More', 'top-store-pro' ) );
		$read_more_classes        = apply_filters( 'top_store_post_read_more_class', array() );
		$post_link = sprintf(
			esc_html( '%s' ),
			'<a class="' . implode( ' ', $read_more_classes ) . ' thunk-readmore button " href="' . esc_url( get_permalink() ) . '"> ' . the_title( '<span class="screen-reader-text">', '</span>', false ) . $top_store_pro_read_more_text . '</a>'
		);
...
the_posts_pagination(array(
    'mid_size'  => 2,
    'prev_text' => __( '', 'top-store-pro' ),
    'next_text' => __( '', 'top-store-pro' ),
));
}
