...
class WXR_Importer extends WP_Importer {
	...
	protected function get_reader( $file ) {
		...
		if ( ! $status ) {
			return new WP_Error( 'wxr_importer.cannot_parse', __( 'Could not open the file for parsing', 'top-store-pro' ) );
		}
				...
				case 'wp:wxr_version':
					...
					if ( version_compare( $this->version, self::MAX_WXR_VERSION, '>' ) ) {
						$this->logger->warning( sprintf(
							__( 'This WXR file (version %1$s) is newer than the importer (version %2$s) and may not be supported. Please consider updating.', 'top-store-pro' ),
							$this->version,
	...
	protected function import_start( $file ) {
		if ( ! is_file( $file ) ) {
			return new WP_Error( 'wxr_importer.file_missing', __( 'The file does not exist, please try again.', 'top-store-pro' ) );
		}
	...
	public function set_user_mapping( $mapping ) {
		foreach ( $mapping as $map ) {
			if ( empty( $map['old_slug'] ) || empty( $map['old_id'] ) || empty( $map['new_id'] ) ) {
				$this->logger->warning( __( 'Invalid author mapping', 'top-store-pro' ) );
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		if ( ! $post_type_object ) {
			$this->logger->warning( sprintf(
				__( 'Failed to import "%1$s": Invalid post type %2$s', 'top-store-pro' ),
				$data['post_title'],
				$data['post_type']
			) );
			return false;
		...
		if ( $post_exists ) {
			$this->logger->info( sprintf(
				__( '%1$s "%2$s" already exists.', 'top-store-pro' ),
		...
		if ( is_wp_error( $post_id ) ) {
			$this->logger->error( sprintf(
				__( 'Failed to import "%1$s" (%2$s)', 'top-store-pro' ),
				$data['post_title'],
				$post_type_object->labels->singular_name
			) );
		...
		$this->logger->info( sprintf(
			__( 'Imported "%1$s" (%2$s)', 'top-store-pro' ),
			$data['post_title'],
			$post_type_object->labels->singular_name
		) );
		$this->logger->debug( sprintf(
			__( 'Post %1$d remapped to %2$d', 'top-store-pro' ),
			$original_id,
			$post_id
		) );
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		if ( ! $info ) {
			return new WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'top-store-pro' ) );
		}
	...
	protected function process_author( $data, $meta ) {
		...
		if ( is_wp_error( $user_id ) ) {
			$this->logger->error( sprintf(
				__( 'Failed to import user "%s"', 'top-store-pro' ),
				$userdata['user_login']
			) );
		...
		$this->logger->info( sprintf(
			__( 'Imported user "%s"', 'top-store-pro' ),
			$userdata['user_login']
		) );
		$this->logger->debug( sprintf(
			__( 'User %1$d remapped to %2$d', 'top-store-pro' ),
			$original_id,
			$user_id
		) );
	...
	protected function process_term( $data, $meta ) {
		...
		$this->logger->debug( sprintf(
			__( 'Term %1$d remapped to %2$d', 'top-store-pro' ),
			$original_id,
			$term_id
		) );
	...
	protected function fetch_remote_file( $url, $post ) {
		...
		if ( $code !== 200 ) {
			...
			return new WP_Error(
				'import_file_error',
				sprintf(
					__( 'Remote server returned %1$d %2$s for %3$s', 'top-store-pro' ),
					$code,
		...
		if ( isset( $headers['content-length'] ) && $filesize !== (int) $headers['content-length'] ) {
			unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', __( 'Remote file is incorrect size', 'top-store-pro' ) );
		}

		if ( 0 === $filesize ) {
			unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', __( 'Zero size file downloaded', 'top-store-pro' ) );
		}

		$max_size = (int) $this->max_attachment_size();
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			unlink( $upload['file'] );
			$message = sprintf( __( 'Remote file is too large, limit is %s', 'top-store-pro' ), size_format( $max_size ) );
	...
	protected function post_process_posts( $todo ) {
		foreach ( $todo as $post_id => $_ ) {
			$this->logger->debug( sprintf(
				...
				__( 'Running post-processing for post %d', 'top-store-pro' ),
				$post_id
			) );
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['post'][ $parent_id ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the post parent for "%1$s" (post #%2$d)', 'top-store-pro' ),
						get_the_title( $post_id ),
						$post_id
					) );
					$this->logger->debug( sprintf(
						__( 'Post %1$d was imported with parent %2$d, but could not be found', 'top-store-pro' ),
						$post_id,
						$parent_id
					) );
				}
			...
			if ( ! empty( $author_slug ) ) {
				...
				if ( isset( $this->mapping['user_slug'][ $author_slug ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the author for "%1$s" (post #%2$d)', 'top-store-pro' ),
						get_the_title( $post_id ),
						$post_id
					) );
					$this->logger->debug( sprintf(
						__( 'Post %1$d was imported with author "%2$s", but could not be found', 'top-store-pro' ),
						$post_id,
						$author_slug
					) );
				}
			...
			if ( empty( $data ) ) {
				$this->logger->debug( sprintf(
					__( 'Post %d was marked for post-processing, but none was required.', 'top-store-pro' ),
					$post_id
				) );
				continue;
			}
			...
			if ( is_wp_error( $result ) ) {
				$this->logger->warning( sprintf(
					__( 'Could not update "%1$s" (post #%2$d) with mapped data', 'top-store-pro' ),
					get_the_title( $post_id ),
					$post_id
				) );
				$this->logger->debug( $result->get_error_message() );
	...
	protected function post_process_menu_item( $post_id ) {
		...
		if ( ! empty( $menu_object ) ) {
			...
			$this->logger->warning( sprintf(
				__( 'Could not find the menu object for "%1$s" (post #%2$d)', 'top-store-pro' ),
				get_the_title( $post_id ),
				$post_id
			) );
			$this->logger->debug( sprintf(
				__( 'Post %1$d was imported with object "%2$d" of type "%3$s", but could not be found', 'top-store-pro' ),
				$post_id,
				$menu_object_id,
				$menu_item_type
			) );
		...
		foreach ( $todo as $comment_id => $_ ) {
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['comment'][ $parent_id ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the comment parent for comment #%d', 'top-store-pro' ),
						$comment_id
					) );
					$this->logger->debug( sprintf(
						__( 'Comment %1$d was imported with parent %2$d, but could not be found', 'top-store-pro' ),
						$comment_id,
						$parent_id
					) );
				}
			...
			if ( ! empty( $author_id ) ) {
				...
				if ( isset( $this->mapping['user'][ $author_id ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the author for comment #%d', 'top-store-pro' ),
						$comment_id
					) );
					$this->logger->debug( sprintf(
						__( 'Comment %1$d was imported with author %2$d, but could not be found', 'top-store-pro' ),
						$comment_id,
						$author_id
					) );
				}
			...
			if ( empty( $result ) ) {
				$this->logger->warning( sprintf(
					__( 'Could not update comment #%d with mapped data', 'top-store-pro' ),
					$comment_id
				) );
				continue;
			}
