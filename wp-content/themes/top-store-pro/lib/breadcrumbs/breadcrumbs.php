 ...
 *
 * CAUTION: This is modified version of original breadcrumb library. Please do not replace when new
 * version of library is released.
 *
 * NOTE: This needs to be done in theme until https://github.com/justintadlock/breadcrumb-trail/issues/22 issue is fixed.
 *
...
/**
 * Shows a breadcrumb for all types of pages.  This is a wrapper function for the top_store_Breadcrumb_Trail class,
 * which should be used in theme templates.
 *
 * @since  0.1.0
 * @access public
 * @param  array $args Arguments to pass to top_store_Breadcrumb_Trail.
 * @return void|string
 */
function top_store_breadcrumb_trail( $args = array() ) {

	$breadcrumb = apply_filters( 'top_store_breadcrumb_trail_object', null, $args );

	if ( !is_object( $breadcrumb ) )
		$breadcrumb = new top_store_Breadcrumb_Trail( $args );
 ...
 * @access public
 */
class top_store_Breadcrumb_Trail {
	...
	/**
	 * Sets up the breadcrumb trail properties.  Calls the `top_store_Breadcrumb_Trail::add_items()` method
	 ...
	 *     @type array     $labels         Text labels. @see top_store_Breadcrumb_Trail::set_labels()
	 *     @type array     $post_taxonomy  Taxonomies to use for post types. @see top_store_Breadcrumb_Trail::set_post_taxonomy()
	...
	public function __construct( $args = array() ) {
		...
		$this->args = apply_filters( 'top_store_breadcrumb_trail_args', wp_parse_args( $args, $defaults ) );
	...
	public function trail(){
		...
		if ( 0 < $item_count ) {
			...
			if ( true === $this->args['show_browse'] )
				$breadcrumb .= sprintf( '<h2 class="trail-browse"></h2>', $this->labels['browse'] );

			// Open the unordered list.
			$breadcrumb .= '<ul class="thunk-breadcrumb trail-items">';

			// Add the number of items and item list order schema.
			// We do not add meta tag due to validation issue.
			...
			$breadcrumb = sprintf(
				'<%1$s aria-label="%2$s" class="breadcrumb-trail breadcrumbs">%3$s%4$s%5$s</%1$s>',
		...
		$breadcrumb = apply_filters( 'top_store_breadcrumb_trail', $breadcrumb, $this->args );
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:',                               'top-store-pro' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'top-store-pro' ),
			'home'                => esc_html__( 'Home',                                  'top-store-pro' ),
			'error_404'           => esc_html__( '404 Not Found',                         'top-store-pro' ),
			'archives'            => esc_html__( 'Archives',                              'top-store-pro' ),
			// Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
			'search'              => esc_html__( 'Search results for &#8220;%s&#8221;',   'top-store-pro' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s',                               'top-store-pro' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s',                             'top-store-pro' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s',                               'top-store-pro' ),
		...
		$this->labels = apply_filters( 'top_store_breadcrumb_trail_labels', wp_parse_args( $this->args['labels'], $defaults ) );
	}
	...
	protected function set_post_taxonomy() {
		...
		$this->post_taxonomy = apply_filters( 'top_store_breadcrumb_trail_post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
	}
	...
	protected function add_items() {
		...
		$this->items = array_unique( apply_filters( 'top_store_breadcrumb_trail_items', $this->items, $this->args ) );
	}
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'top-store-pro' ) ) );
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'top-store-pro' ) ) );
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'top-store-pro' ) ) );
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'top-store-pro' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'top-store-pro' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'top-store-pro' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'top-store-pro' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'top-store-pro' ) ) );
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'top-store-pro' ) ) ) );

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'top-store-pro' ) ) ) );

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'top-store-pro' ) ) ) );
